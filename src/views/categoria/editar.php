...
<body>
    ...
    <form method="POST" action="<?= BASE_URL ?>categorias/editar/<?= (int)$categoria->id ?>">
        ...
        <input type="text" id="name" name="name" value="<?= htmlspecialchars($old['name'] ?? $categoria->name, ENT_QUOTES, 'UTF-8') ?>" required>
        <?php if (!empty($errores['name'])): ?>
            <div class="error"><?= htmlspecialchars($errores['name'], ENT_QUOTES, 'UTF-8') ?></div>
        <?php endif; ?>

        <label for="description">Descripción</label>
        <textarea id="description" name="description"><?= htmlspecialchars($old['description'] ?? $categoria->description, ENT_QUOTES, 'UTF-8') ?></textarea>
